...
export function ActiveServices() {
  ...
  useEffect(() => {
    apiRequest("/services")
      .then(async ({ services: items }) => {
        const openServices = items.filter(
          (service) => !service.archived && service.queueStatus !== "closed"
        );
        const statuses = await Promise.all(
          openServices.map((service) => apiRequest(`/queues/${service.id}/status`))
        );
        setServices(
          openServices.map((service, index) => ({ ...service, ...statuses[index] }))
        );
      })
      .catch(() => {});
  }, []);
      ...
      <CardContent className="max-h-195 space-y-3 overflow-y-auto pr-1">
          ...
          <div key={service.id} className="rounded-md border p-3">
            <div className="flex justify-between">
              ...
              <Badge variant="secondary">
                {service.status === "open"
                  ? service.openLaneCount > 1
                    ? `${service.openLaneCount} lanes`
                    : "Open"
                  : "Closed"}
              </Badge>
            </div>
        ...
        {!services.length && (
          <p className="text-sm text-muted-foreground">
            No open services are available right now.
          </p>
        )}
      </CardContent>
    </Card>
